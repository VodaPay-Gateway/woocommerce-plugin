...
/**
 * VodaPayGatewayConfig class.
 */
class VodaPayGatewayConfig
{
    /**
     * Config tags
     */
     ...
     * Constructor.
     *
     * @param VodaPayGateway $gateway VodaPay Digital Payment Gateway gateway object.
     */
    public function __construct(VodaPayGateway $gateway, $order = "")
    {
        $this->order   = $order;
    ...
    public function should_use_outlet_override_ref(): bool
    {
        ...
        if (is_array($overriddenCurrencies)) {
            ...
            foreach ($overriddenCurrencies as $overriddenCurrency) {
                if ($orderCurrency === SettingsVodaPay::$currencies[intval($overriddenCurrency)]) {
                    return true;
                }
            }
        }
